...
import { Home, Trophy, Medal, Crown, Sparkles, Loader2, Globe2, TrendingUp, CloudOff } from 'lucide-react';
import { subscribeToLeaderboard, getBadgeDefinitions, isCloudEnabled } from '../services/statsService';
import { LeaderboardEntry } from '../types';

interface LeaderboardScreenProps {
  onBack: () => void;
  currentUser?: string; // This is now the UID
}
      ...
      <div className="flex items-end justify-center gap-3 sm:gap-8 mb-16 mt-8 px-2">
        {top3[1] && (
          <div className="flex flex-col items-center animate-fade-in-up [animation-delay:200ms]">
            ...
            <div className="bg-white/60 backdrop-blur-md px-4 py-4 rounded-t-3xl shadow-lg w-24 sm:w-36 text-center h-24 flex flex-col justify-center">
        ...
        {top3[0] && (
          <div className="flex flex-col items-center animate-pop-in relative z-10">
            ...
            <div className="bg-white px-5 py-6 rounded-t-[2.5rem] shadow-2xl w-32 sm:w-48 text-center h-36 flex flex-col justify-center ring-4 ring-yellow-400/10">
        ...
        {top3[2] && (
          <div className="flex flex-col items-center animate-fade-in-up [animation-delay:400ms]">
            ...
            <div className="bg-white/60 backdrop-blur-md px-4 py-4 rounded-t-3xl shadow-lg w-24 sm:w-36 text-center h-20 flex flex-col justify-center">
    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 via-white to-gray-50 flex flex-col items-center p-4 sm:p-8 overflow-y-auto">
      <div className="w-full max-w-4xl">
        <div className="flex items-center justify-between mb-12">
          <button onClick={onBack} className="bg-white p-4 rounded-3xl shadow-sm text-gray-500 hover:text-indigo-600 transition-all active:scale-90">
            <Home size={24} />
          </button>
          <div className="text-center">
            <h1 className="text-3xl sm:text-5xl font-black text-slate-800 flex items-center justify-center gap-3">
              <div className="bg-indigo-600 p-3 rounded-2xl text-white shadow-xl rotate-3"><Trophy size={32} /></div>
              لوحة الأبطال
            </h1>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isCurrentUser && <span className="bg-white/20 text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase">أنت</span>}
                        </div>
                        <div className={`text-[10px] font-bold flex items-center gap-1.5 mt-1 ${isCurrentUser ? 'text-indigo-200' : 'text-slate-400'}`}>
                          <TrendingUp size={12} /> آخر نشاط: {player.lastActive}
                        </div>
                      </div>
                      <div className="hidden sm:flex gap-1.5 px-4 border-x border-gray-100/10">
                        {badges.slice(0, 4).map(b => (
                          <div key={b.id} className="text-2xl hover:scale-125 transition-transform" title={b.name}>{b.icon}</div>
                        ))}
                      </div>
                      <div className="text-right min-w-[80px]">
                        ...
                        <div className={`text-[9px] font-black uppercase mt-1 ${isCurrentUser ? 'text-indigo-200' : 'text-indigo-300'}`}>نقطة</div>
                      </div>
                    </div>
                  );
                })
